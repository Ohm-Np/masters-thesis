---
title: "sequential vs parallelization (wdpa polygons)"
...
final01 <- read.csv("data/results/rasters/bolivia/final_01.csv")
...
final001 <- read.csv("data/results/rasters/bolivia/final_001.csv")
...
final0001 <- read.csv("data/results/rasters/bolivia/final_0001.csv")
...
final00005 <- read.csv("data/results/rasters/bolivia/final_00005.csv")
...
_terra extract(1) has created much spikes over the graphs, so lets remove that part and plot others._

```{r 0.0005 degree final plot 2, message=FALSE, warning=FALSE, include=TRUE, fig.width=12}
result_new
```
...
|0.01 degree (~1.11 km) |6.9 MB|1. terra zonal in sequential mode </br> 2. exact extract in sequential mode|
|0.001 degree (~111 m)|691.9 MB|1. for wdpa polygons, exact extract with 16 cores </br> 2. for fishnets, exact extract sequential|
|0.0005 degree (~55.5 m)|2.7 GB|1. for wdpa polygons, exact extract with 16 cores </br> 2. for fishnets, exact extract sequential|
